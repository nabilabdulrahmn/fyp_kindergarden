...
$teacher_classes = dapatkan_senarai_kelas_guru($conn, $teacher_id);

// Ambil senarai kelas guru dengan nama untuk dropdown
$class_options = array();
if (!empty($teacher_classes)) {
    $class_ids_str = implode(',', $teacher_classes);
    $cls_result = $conn->query("SELECT id, class_name FROM classes WHERE id IN ($class_ids_str) ORDER BY class_name");
    if ($cls_result) {
        while ($cls_row = $cls_result->fetch_assoc()) {
            $class_options[] = $cls_row;
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_kehadiran'])) {
    ...
    if (!empty($teacher_classes)) {
        foreach ($all_students as $stud_id) {
            ...
            // Validate that teacher has access to this student via class
            if (sahkan_akses_pelajar($conn, $teacher_id, $stud_id)) {
        ...
        echo "<script>alert('Tiada kebenaran untuk menyimpan kehadiran.'); window.location.href='ambil_kehadiran.php';</script>";
        exit();
    }
}
...
if (empty($teacher_classes)) {
    $sql_kiri = "SELECT * FROM students WHERE 1=0";
    $sql_kanan = "SELECT a.status, s.full_name, c.class_name FROM attendance a JOIN students s ON a.student_id = s.id JOIN student_classes sc ON s.id = sc.student_id JOIN classes c ON sc.class_id = c.id WHERE 1=0";
} else {
    $class_ids_str = implode(',', $teacher_classes);
    
    $sql_kiri = "SELECT DISTINCT s.*, c.class_name 
                 FROM students s 
                 INNER JOIN student_classes sc ON s.id = sc.student_id 
                 INNER JOIN classes c ON sc.class_id = c.id 
                 WHERE s.status = 'Active' 
                   AND sc.class_id IN ($class_ids_str)";
    if ($filter_class > 0 && in_array($filter_class, $teacher_classes)) {
        $sql_kiri .= " AND sc.class_id = $filter_class";
    }
    $sql_kiri .= " ORDER BY s.full_name ASC";
    
    // --- DATA KANAN: REKOD KEHADIRAN HARIAN ---
    $sql_kanan = "SELECT DISTINCT a.status, s.full_name, c.class_name 
                  FROM attendance a 
                  ...
                  WHERE a.date = '$filter_date' 
                    AND sc.class_id IN ($class_ids_str)";
    if ($filter_class > 0 && in_array($filter_class, $teacher_classes)) {
        $sql_kanan .= " AND sc.class_id = $filter_class";
    }
...
<main class="main-content-shifted" style="padding: 20px;">
    ...
    <div class="header-bar">
        ...
        <form method="GET" action="ambil_kehadiran.php">
            ...
            <label style="font-weight:bold; color:#555;">Kelas:</label>
            <select name="class_id">
                <option value="">Semua Kelas</option>
                <?php foreach ($class_options as $cls): ?>
                    <option value="<?php echo $cls['id']; ?>" <?php if($filter_class == $cls['id']) echo 'selected'; ?>><?php echo htmlspecialchars($cls['class_name']); ?></option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="main-container">
        ...
        <div class="panel left-panel">
            ...
            <form method="POST" action="ambil_kehadiran.php?date=<?php echo $filter_date; ?>&class_id=<?php echo $filter_class; ?>">

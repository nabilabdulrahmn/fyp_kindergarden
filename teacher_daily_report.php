...
$approved_mods = [];
$r_mods = $conn->query("SELECT module FROM teacher_modules WHERE teacher_id = $teacher_id AND status = 'Approved'");
if ($r_mods) {
    while ($m_row = $r_mods->fetch_assoc()) {
        $approved_mods[] = $m_row['module'];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_laporan'])) {
    ...
    // verify teacher has access to class_id
    if (!empty($approved_mods)) {
        ...
        $sec_check = $conn->query("SELECT id FROM classes WHERE id = $class_id AND teacher_id = $teacher_id AND module IN ($modules_list)");
        if (!$sec_check || $sec_check->num_rows == 0) {
            $msg = "<div class='alert error'>Ralat: Anda tiada kebenaran untuk kelas ini.</div>";
            $class_id = 0; // invalidate
        }
    } else {
        $msg = "<div class='alert error'>Ralat: Anda tiada kebenaran untuk mana-mana kelas.</div>";
        $class_id = 0;
    }
...
// --- AMBIL SENARAI KELAS ---
if (empty($approved_mods)) {
    $classes_result = $conn->query("SELECT id, class_name, module FROM classes WHERE 1=0");
} else {
    $modules_list = "'" . implode("','", array_map(function($m) use ($conn) { return mysqli_real_escape_string($conn, $m); }, $approved_mods)) . "'";
    $classes_result = $conn->query("SELECT id, class_name, module FROM classes WHERE teacher_id = '$teacher_id' AND module IN ($modules_list) ORDER BY class_name ASC");
}

// Ambil laporan harian yang telah dihantar oleh guru ini
if (empty($approved_mods)) {
    $sql_history = "SELECT dr.*, c.class_name, c.module FROM daily_reports dr JOIN classes c ON dr.class_id = c.id WHERE 1=0";
} else {
    $modules_list = "'" . implode("','", array_map(function($m) use ($conn) { return mysqli_real_escape_string($conn, $m); }, $approved_mods)) . "'";
    $sql_history = "SELECT dr.*, c.class_name, c.module 
                    FROM daily_reports dr 
                    JOIN classes c ON dr.class_id = c.id 
                    WHERE dr.teacher_id = '$teacher_id' AND c.module IN ($modules_list)

...
// --- QUERY ALERGEN & ALAHAN PELAJAR (KATEGORI: AKSES TERHAD IKUT KELAS CIKGU INI SAHAJA) ---
$allergies_list = [];
$allergy_sql = "
    ...
    WHERE c.teacher_id = '$teacher_id' AND s.status = 'Active' AND s.allergies IS NOT NULL AND s.allergies != ''
";
$allergy_res = $conn->query($allergy_sql);

// Fallback: Jika guru belum ada kelas, tunjukkan semua amaran alahan pusat untuk memastikan tiada kemalangan pemakanan berlaku
if (!$allergy_res || $allergy_res->num_rows == 0) {
    $allergy_sql = "
        SELECT s.full_name, s.module, s.allergies, s.health_record, 'Semua Kelas / Umum' as class_name 
        FROM students s 
        WHERE s.status = 'Active' AND s.allergies IS NOT NULL AND s.allergies != ''
    ";
    $allergy_res = $conn->query($allergy_sql);
}

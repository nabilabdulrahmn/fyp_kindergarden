...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_visitor'])) {
    ...
    $sql = "INSERT INTO visitor_logs (visitor_name, ic_number, purpose, visiting_whom, check_in, badge_number, recorded_by) 
            VALUES ('$visitor_name', '$ic_number', '$purpose', '$visiting_whom', '$check_in', '$badge_number', $user_id)";
    if ($conn->query($sql)) {
            ...
            <tbody>
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td>
                                <strong><?php echo htmlspecialchars($row['visitor_name']); ?></strong><br>

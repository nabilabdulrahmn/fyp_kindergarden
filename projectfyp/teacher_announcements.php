...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['hantar_pengumuman'])) {
    ...
    // Semak sekuriti: Adakah guru ini benar-benar mengajar kelas ini?
    $check_class = $conn->query("SELECT id FROM classes WHERE id = '$class_id' AND teacher_id = '$teacher_id'");
...
// --- AMBIL KELAS YANG DIAJAR OLEH GURU INI SAHAJA (LIMITASI CAPASITI AKSES GURU) ---
$my_classes = [];
$classes_res = $conn->query("SELECT id, class_name, module FROM classes WHERE teacher_id = '$teacher_id'");
if ($classes_res) {
...
$sql_history = "SELECT a.*, c.class_name 
                ...
                WHERE a.author_id = '$user_id' AND a.scope = 'class' 

...
// ==========================================
$daily_class = isset($_GET['daily_class']) ? $_GET['daily_class'] : 'Tadika 4-5 Tahun';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_jadual'])) {
    ...
    foreach ($all_activity_ids as $act_id) {
        ...
        $conn->query("UPDATE activity_schedules SET status='$status' WHERE id='$act_id'");
    }
...
if ($hist_class != '') $sql_history .= " AND class_group = '$hist_class'";

...
if (in_array('Tadika', $approved_mods)) {
    $valid_groups[] = 'Tadika 4-5 Tahun';
    $valid_groups[] = 'Tadika 6 Tahun';
}
if (in_array('KAFA Care', $approved_mods)) {
    $valid_groups[] = 'KAFA Care';
}
if (in_array('Taska', $approved_mods)) {
    $valid_groups[] = 'Aktiviti Taska';
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_rph'])) {
    ...
    if (!in_array($class_group, $valid_groups)) {
        echo "<script>alert('Ralat: Anda tiada kebenaran untuk kumpulan kelas ini.'); window.location.href='lesson_plan.php';</script>";
        exit();
    }
...
if (empty($valid_groups)) {
    $sql_history = "SELECT * FROM lesson_plans WHERE 1=0";
} else {
    $groups_list = "'" . implode("','", array_map(function($g) use ($conn) { return mysqli_real_escape_string($conn, $g); }, $valid_groups)) . "'";
    $sql_history = "SELECT * FROM lesson_plans WHERE teacher_id = '$user_id' AND class_group IN ($groups_list) ORDER BY teaching_date DESC LIMIT 50";
}
